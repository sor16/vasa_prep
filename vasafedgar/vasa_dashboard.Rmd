...
activities <- update_activities(con,users) %>%
              mutate(type=ifelse(type %in% c('Hike','Golf'),'Walk',type),
                     type=ifelse(type %in% c('Ride','VirtualRide'),'Walk',type)) #hack to allow golf activities
# activities <- load_data(con,'Activity') %>%
#               mutate(type=ifelse(type %in% c('Hike','Golf'),'Walk',type)) #hack to allow golf activities
